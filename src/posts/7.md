...
Next, change your image from `ghcr.io/imputnet/cobalt` to `ghcr.io/zimpatrick/cobalt:staging`. This image is a fork of cobalt to make changes to allow YouTube (and other services) to work. This is an up to date fork by [patriick](https://patriick.dev/). You can see the source code [here](https://github.com/zImPatrick/cobalt).
...
<blockquote class="alert-box">
<p><code>YOUTUBE_PLAYER_ID</code> needs to be updated often with new player IDs. You can find them by accessing <code>/tmp/youtubei.js</code> in your container. You should use the latest ones from that folder.</p>
<p>YouTube is not 100% guaranteed to work.</p>
</blockquote>
